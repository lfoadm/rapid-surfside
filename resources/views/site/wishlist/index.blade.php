            ...
            <div class="cart-table__wrapper">
                                    ...
                                    <div class="row">
                                        <div class="col-6">
                                            <form action="{{ route('wishlist.move.to.cart', ['rowId'=>$item->rowId]) }}" method="post">
                                                @csrf
                                                <button type="submit" class="btn btn-sm btn-warning">+ Carrinho</button>
                                            </form>
                                        </div>                                    
                                        <div class="col-6">
                ...
                <div class="cart-table-footer">
                    <form method="POST" action="{{ route('wishlist.items.clear') }}" >
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-warning">LIMPAR LISTA DE DESEJOS</button>
                    </form>
                </div>
            </div>
            @else
                <div class="row">
                    <div class="col-md-12">
                        <p>Não há items em sua lista de desejos</p>
                        <a href="{{ route('shop.index') }}" class="btn btn-info">Ir para ofertas</a>
                    </div>
                </div>
            @endif
        </div>
